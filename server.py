...
def getData():
    ...
    return s_data, s_f_data, etc_data, etc_f_data


def task():
    s_data, s_f_data, etc_data, etc_f_data = getData()
    #s_data = [[d1 + datetime.datetime.now().strftime("%H:%M:%S") for d in s_data for d1 in d]]
    #print(s_data)
    socketio.emit('value update', {"s_data":s_data, "s_f_data":s_f_data, "etc_data":etc_data, "etc_f_data":etc_f_data}, broadcast=True)
